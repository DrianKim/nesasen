            ...
            <form action="{{ route('guru.izin.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <div class="upload-container">
                        ...
                        @if (isset($izinGuru) && !empty($izinGuru->lampiran))
                            <div class="mt-3">
                                <p class="text-muted">Lampiran yang sudah diupload:</p>
                                @if (Str::endsWith(strtolower($izinGuru->lampiran), ['.jpg', '.jpeg', '.png', '.gif', '.bmp']))
                                    <img src="{{ asset('storage/' . $izinGuru->lampiran) }}" alt="Lampiran"
                                        class="mt-2 img-fluid" style="max-width: 200px;">
                                @else
                                    <div class="file-info-existing">
                                        <i class="fas fa-file-alt"></i>
                                        <span>{{ basename($izinGuru->lampiran) }}</span>
                                    </div>
                                @endif
                            </div>
                        @endif
